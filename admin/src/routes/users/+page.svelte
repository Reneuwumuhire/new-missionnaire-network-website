...
<div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
	...
	<button
		...
		class="admin-btn-primary justify-center whitespace-nowrap"
	>
			...
			<div class="grid gap-4 sm:grid-cols-3">
				<div>
					...
					<input
						...
						type="text"
						required
						class="admin-input {createFieldError?.field === 'name'
					...
					{#if createFieldError?.field === 'name'}
						<p id="name-error" class="mt-1.5 text-xs text-red-600">
							{createErrorMessage(createFieldError.code)}
						</p>
					{/if}
				</div>
				<div>
					...
					<input
						...
						type="email"
						required
						class="admin-input {createFieldError?.field === 'email'
							? 'border-red-400 focus:border-red-500 focus:ring-red-200'
							: ''}"
					...
					{#if createFieldError?.field === 'email'}
						<p id="email-error" class="mt-1.5 text-xs text-red-600">
							{createErrorMessage(createFieldError.code)}
						</p>
					{/if}
				</div>
				<div>
					...
					{#if createFieldError?.field === 'role'}
						<p id="role-error" class="mt-1.5 text-xs text-red-600">
							{createErrorMessage(createFieldError.code)}
						</p>
					{/if}
				</div>
...
<!-- Users table -->
<div class="users-list overflow-hidden border border-stone-200/60 bg-white/40">
	<table class="users-table w-full text-left text-sm">
		<thead>
		...
		<tbody>
			...
			{:then users}
				{#each users as user}
					...
					<tr class="user-row border-b border-stone-50 transition-colors hover:bg-cream/30">
						<td data-label={$t('users.table.user')} class="px-5 py-4">
						...
						<td data-label={$t('users.role')} class="px-5 py-4">
							<span
						...
						<td data-label={$t('users.table.permissions')} class="px-5 py-4">
						...
						<td data-label={$t('users.table.status')} class="px-5 py-4">
							<span
						...
						<td data-label={$t('users.table.lastLogin')} class="px-5 py-4 text-stone-500"
							>{formatDate(user.last_login)}</td
						>
						<td data-label={$t('users.table.actions')} class="px-5 py-4">
							...
							<td colspan="6" class="px-5 py-4">
								...
								>
									...
									>
										...
										<div>
											<span class="text-sm font-medium text-stone-700"
												>{$t('users.permRecordings.label')}</span
											>
									...
									>
										...
										<div>
											<span class="text-sm font-medium text-stone-700"
												>{$t('users.permQAnswer.label')}</span
											>
											<p class="text-[10px] text-stone-400">
									...
									>
										...
										<div>
											<span class="text-sm font-medium text-stone-700"
												>{$t('users.permQModerate.label')}</span
											>
											<p class="text-[10px] text-stone-400">
			...
			{:catch}
				<tr>
					<td colspan="6" class="px-5 py-12 text-center">
						...
						<button
							class="admin-btn-secondary admin-btn-compact mt-4"
...
<style>
	...
	@container (max-width: 1050px) {
		...
		.user-row td:nth-child(3)::before,
		.user-row td:nth-child(5)::before,
		.user-row td:nth-child(6)::before {
		...
		.user-row td:last-child > div {
			justify-content: flex-start;
			flex-wrap: wrap;
		}
		.user-row :global(button) {
			min-height: 44px;
			min-width: 44px;
		}
		.user-row :global(form) {
			flex-wrap: wrap;
		}
	}
</style>
