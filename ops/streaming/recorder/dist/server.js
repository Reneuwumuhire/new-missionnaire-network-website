...
app.post('/stop', async (req, reply) => {
    if (!isRecording())
        return reply.code(409).send({ error: 'not_recording' });
    try {
...
app.post('/retry/:id', async (req, reply) => {
    const { id } = req.params;
    ...
    if (doc.status === 'ready')
        return reply.code(409).send({ error: 'already_ready' });
    try {
        ...
        return reply.code(500).send({ error: 'retry_failed', message: err.message });
    }
});
async function main() {
