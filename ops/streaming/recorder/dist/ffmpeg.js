...
import { ensureRecordingsDir, mp3Path, removeRecordingFiles, segmentMp3Path, updateSidecar, writeSidecar } from './sidecar.js';
import { getBroadcastSnapshot, insertRecordingStarting, markRecordingFailed, markRecordingReady, markRecordingStopping } from './mongo.js';
...
function ffmpegArgsFor(outPath) {
    return [
        '-nostdin',
        '-hide_banner',
        '-loglevel',
...
async function startSegment(rec) {
    ...
    proc.once('exit', (code, signal) => {
        ...
        if (!current || current.idStr !== rec.idStr || current.proc !== proc)
            return;
        if (current.stopping || current.abandoned)
            return;
        const segmentLifetimeMs = Date.now() - segment.startedAt.getTime();
        const exitDesc = signal ? `signal=${signal}` : `code=${code}`;
        console.warn(`[recorder] ${rec.idStr} segment ${segIndex} exited unexpectedly after ${segmentLifetimeMs}ms (${exitDesc}); scheduling restart`);
        scheduleRestart(current, segmentLifetimeMs);
    });
    console.log(`[recorder] ${rec.idStr} segment ${segIndex} started -> ${segPath}`);
}
function scheduleRestart(rec, lastSegmentLifetimeMs) {
    ...
    if (wasHealthy) {
        ...
        if (rec.recoveryStartedAt === null)
            rec.recoveryStartedAt = new Date();
    }
    const inRecoveryFor = Date.now() - rec.recoveryStartedAt.getTime();
    if (inRecoveryFor >= RECOVERY_GIVE_UP_MS) {
        console.error(`[recorder] ${rec.idStr} giving up after ${Math.round(inRecoveryFor / 1000)}s in recovery — finalizing what we have`);
        rec.abandoned = true;
        // Finalize asynchronously with whatever segments we captured.
        void finalizeAbandoned(rec).catch((err) => console.error(`[recorder] abandoned finalize failed for ${rec.idStr}`, err));
        return;
    }
    const backoffIdx = Math.min(rec.consecutiveFailures, RESTART_BACKOFF_MS.length) - 1;
    const delay = backoffIdx < 0
    ...
    console.log(`[recorder] ${rec.idStr} restart scheduled in ${delay}ms (consecutive failures: ${rec.consecutiveFailures})`);
}
export async function startRecording(params) {
    ...
    try {
        ...
        try {
            await markRecordingFailed(id, `start failed: ${err.message}`);
        }
        catch (markErr) {
...
async function finalizeAbandoned(rec) {
    rec.stopping = true;
    clearTimeout(rec.safetyTimer);
    if (rec.restartTimer) {
        clearTimeout(rec.restartTimer);
    ...
    const endedAt = new Date();
    await finalizeAndUpload(rec, endedAt).catch((err) => console.error(`[recorder] abandoned finalize+upload failed for ${rec.idStr}`, err));
    if (active && active.idStr === rec.idStr)
        active = null;
}
/** Concat segments → canonical mp3, mark stopping in DB, then kick off upload
 *  in the background so /stop returns fast. */
async function finalizeAndUpload(rec, endedAt) {
    ...
    try {
        await concatSegments(rec.idStr, rec.segments.map((s) => s.path));
    }
    catch (err) {
        console.error(`[recorder] concat failed for ${rec.idStr}`, err);
        try {
            await markRecordingFailed(rec.id, `concat failed: ${err.message}`);
        }
        catch (markErr) {
            console.error('[recorder] markRecordingFailed failed', markErr);
        }
    ...
    if (probedSec !== null && wallClockSec - probedSec > 30) {
        console.warn(`[recorder] ${rec.idStr} audio is ${wallClockSec - probedSec}s shorter than wall clock (probed=${probedSec}s, wall=${wallClockSec}s, segments=${rec.segments.length}) — gaps from upstream loss`);
    }
    try {
...
async function finalizeUpload(rec) {
    try {
        ...
        await markRecordingFailed(rec.id, reason).catch((e) => console.error('[recorder] markRecordingFailed also failed', e));
        throw err;
    }
}
