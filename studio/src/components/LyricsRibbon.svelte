<script lang="ts">
	...
	const styleFor = (offset: number) =>
		offset === 0
			...
			: Math.abs(offset) === 1
				? 'text-[13px] leading-tight text-muted'
				: 'text-[12px] leading-tight text-muted';
</script>
...
>
	<div class="flex w-16 shrink-0 flex-col gap-1">
		<span class="text-[12px] font-semibold uppercase tracking-[0.18em] text-muted">
			{t('dock.lyrics')}
		</span>
		<button
			class="w-fit px-1.5 py-px text-[12px] font-semibold uppercase tracking-wider transition-colors {lyrics.onAir
	...
	<div class="relative min-w-0 flex-1 self-stretch">
		{#if linesSource.length === 0}
			<p class="flex h-full items-center text-[13px] text-muted">{t('lyrics.emptyList')}</p>
		{:else}
	...
	{#if lyrics.mode === 'manual' && linesSource.length > 0}
		<div class="flex shrink-0 items-center gap-1">
			<button class="studio-chip" onclick={() => step(-1)} aria-label={t('lyrics.previous')}
				>←</button
			>
			<button class="studio-btn-primary h-8 px-3 text-[12px]" onclick={() => step(1)}>
				{t('lyrics.next')}
			</button>
		</div>
	...
	{#if !live.current && lyrics.onAir && linesSource.length > 0}
		<span class="shrink-0 text-[12px] text-muted">—</span>
	{/if}

	<div
		class="ml-2 flex h-[76px] w-[420px] shrink-0 overflow-hidden border border-ink-600 bg-ink-950/70"
	>
		<div class="flex min-w-0 flex-1 flex-col justify-center border-r border-ink-600 px-5">
			<span
				class="flex items-center gap-2 text-[12px] font-semibold tracking-[0.18em] {liveSession.activeStartedAt
					? 'text-danger'
					: broadcast.startedAt
						? 'text-warning'
						: 'text-muted'}"
			>
				<span
					class="h-1.5 w-1.5 rounded-full {liveSession.activeStartedAt
		...
		<div class="grid w-[190px] grid-cols-2 divide-x divide-ink-600">
			<div class="flex flex-col items-center justify-center">
				<span class="text-[12px] font-semibold tracking-[0.16em] text-muted">LOCAL</span>
			...
			<div class="flex flex-col items-center justify-center bg-red-500/[0.03]">
				<span
					class="flex items-center gap-1.5 text-[12px] font-semibold tracking-[0.16em] {recording.startedAt
						? 'text-danger'
						: 'text-muted'}"
				>
					{#if recording.startedAt}<span class="h-1.5 w-1.5 animate-pulse rounded-full bg-red-500"
						></span>{/if}
					REC
				</span>
				<span class="font-mono text-[14px] {recording.startedAt ? 'text-danger' : 'text-muted'}">
