...
export type StudioEnvConfig = Partial<
	Pick<Settings, 'mainSiteUrl' | 'adminSiteUrl' | 'recorderUrl' | 'recorderToken'>
> & {
...
function rtmpUrl(value: string | undefined, label: string): string | undefined {
	if (!value) return undefined;
	if (!/^rtmps?:\/\//.test(value) || /[|[\]'"\\\s]/.test(value)) throw new Error(`${label} must use rtmp:// or rtmps://.`);
	return value.replace(/\/+$/, '');
}
...
export function parseStudioEnv(text: string): StudioEnvConfig {
	...
	return {
		mainSiteUrl: webUrl(first(env, 'STUDIO_MAIN_SITE_URL', 'MAIN_SITE_URL', 'PUBLIC_MAIN_URL'), 'Main site URL'),
		adminSiteUrl: webUrl(first(env, 'STUDIO_ADMIN_SITE_URL', 'ADMIN_SITE_URL'), 'Admin site URL'),
		recorderUrl: webUrl(first(env, 'STUDIO_RECORDER_URL', 'RECORDER_URL'), 'Recorder URL'),
		recorderToken: first(env, 'STUDIO_RECORDER_TOKEN', 'RECORDER_TOKEN'),
		missionnaireUrl: rtmpUrl(first(env, 'MISSIONNAIRE_RTMP_URL', 'STUDIO_RTMP_URL', 'RTMP_URL'), 'Missionnaire RTMP URL'),
...
export function mergeEnvDestinations(destinations: Destination[], config: StudioEnvConfig): Destination[] {
