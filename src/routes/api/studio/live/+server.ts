...
async function authorized(request: Request): Promise<{ email: string; name: string; code: string }> {
...
export async function POST({ request, url }) {
	...
	const body = (await request.json().catch(() => ({}))) as {
		...
		description?: string;
		youtubeUrl?: string;
		thumbnailUrl?: string; thumbnailKey?: string;
	...
	if (body.action === 'presign-thumbnail') {
		const ext = ({ 'image/jpeg': 'jpg', 'image/png': 'png', 'image/webp': 'webp', 'image/gif': 'gif' } as Record<string, string>)[body.contentType ?? ''];
		if (!ext || !body.size || body.size > 5 * 1024 * 1024) throw error(400, 'Thumbnail must be a JPEG, PNG, WebP or GIF under 5 MB');
		const key = `broadcast-thumbnails/${Date.now()}-${crypto.randomUUID()}.${ext}`;
		return json({ uploadUrl: await presignUpload(key, body.contentType as string), key, publicUrl: s3Url(key) });
	}
	if (body.action === 'presign-subtitle') {
		if (!body.filename?.toLowerCase().endsWith('.srt') || !body.size || body.size > 2 * 1024 * 1024) throw error(400, 'Subtitle must be an .srt file under 2 MB');
		const key = `subtitles/${Date.now()}-${crypto.randomUUID()}.srt`;
		return json({ uploadUrl: await presignUpload(key, 'text/plain; charset=utf-8'), key, publicUrl: s3Url(key), contentType: 'text/plain; charset=utf-8' });
	}
	if (body.action === 'create') {
		...
		const session = await createStudioScheduledLive({
			title, scheduledAt, createdBy: operator.email, description: body.description?.trim() || null,
			youtubeUrl: body.youtubeUrl?.trim() || null, thumbnailUrl: body.thumbnailUrl ?? null, thumbnailKey: body.thumbnailKey ?? null,
			subtitleUrl: body.subtitleUrl ?? null, subtitleKey: body.subtitleKey ?? null, subtitleFilename: body.subtitleFilename ?? null,
			announce: body.announce === true, reminderEnabled: body.reminderEnabled === true
		});
		return json({ session });
	}
	if (body.action === 'quick-test') {
		...
		const session = await createStudioScheduledLive({
			title: defaultTitle(current.default_title), scheduledAt: new Date(), createdBy: operator.email,
			description: current.default_description, youtubeUrl: current.default_youtube_url,
			thumbnailUrl: current.default_thumbnail_url, thumbnailKey: current.default_thumbnail_s3_key,
			announce: false, reminderEnabled: false, testAccessToken: accessToken
		});
		return json({
			session,
			watchUrl: new URL(`/live/${session.slug}?test=${encodeURIComponent(accessToken)}`, url.origin).toString()
		});
	}
