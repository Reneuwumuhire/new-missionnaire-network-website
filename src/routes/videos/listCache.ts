...
export interface VideosPageCacheEntry {
	videos: YoutubeVideo[];
	liveStatus: YouTubeCachedStatus | null;
	requestedVideo: YoutubeVideo | null;
	skipCount: number;
	hasMore: boolean;
	fetchedAt: number;
}
...
export function getVideosPageCache(key: string): VideosPageCacheEntry | null {
	return cache.get(key) ?? null;
}

export function setVideosPageCache(
	key: string,
...
export function isVideosPageCacheFresh(entry: VideosPageCacheEntry, now = Date.now()): boolean {
