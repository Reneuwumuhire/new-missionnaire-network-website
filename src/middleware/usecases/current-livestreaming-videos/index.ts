...
export default class GetCurrentLiveStreamingEventsUsecase
	implements UseCase<void, YoutubeVideo | null>
{
	async execute(): Promise<Result<YoutubeVideo | null, InternalFailure>> {
		try {
			...
			return Result.err(
				new InternalFailure('Failed to get events that are currently live streamed')
			);
		}
	}
}
